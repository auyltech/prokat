...
class OwnerRegistrationRequestNotifier
  ...
  Future<RegistrationRequestModel?> _fetch(
  ...
  Future<void> _refresh(AuthenticatedSessionScopeKey scope) async {
    final hadData =
        _stateScope == scope && state is AsyncData<RegistrationRequestModel?>;
    ...
    if (!hadData) {
      ...
      if (isAuthenticatedSessionScopeCurrent(ref, scope)) {
        state = next;
        _stateScope = next is AsyncData<RegistrationRequestModel?>
            ? scope
            : null;
      }
      return;
