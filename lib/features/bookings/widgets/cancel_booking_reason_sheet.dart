...
class CancelBookingReasonSheet extends StatefulWidget {
  ...
  static Future<CancelBookingDecision?> show(
    BuildContext context, {
    required BookingModel booking,
    String? useCase,
  }) {
    final l10n = AppLocalizations.of(context)!;
    final isOwner = useCase == 'owner';
    final title = isOwner && booking.status == BookingStatus.created
        ? l10n.rejectOrder
        : l10n.cancelBooking;

    return AppBottomSheet.show<CancelBookingDecision>(
      context,
      title: title,
      contentBuilder: (context) =>
          CancelBookingReasonSheet(booking: booking, useCase: useCase),
    );
  }
