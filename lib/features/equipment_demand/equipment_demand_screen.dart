...
class EquipmentDemandScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EquipmentDemandScreen> createState() => _EquipmentDemandScreenState();
}
...
class _EquipmentDemandScreenState extends ConsumerState<EquipmentDemandScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(equipmentDemandServiceProvider).submit(
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.demandSurveyThankYou)));
      context.pop();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: form.when(
        ...
        error: (_, _) => Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [Text(l10n.demandSurveyLoadError), const SizedBox(height: 12), FilledButton(onPressed: () => ref.invalidate(demandFormProvider(widget.campaignId)), child: const Icon(Icons.refresh))]))),
        data: (data) => ListView(
          padding: const EdgeInsets.all(20),
          children: [
            Text(l10n.demandSurveyQuestionTitle, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            Text(l10n.demandSurveyQuestionSubtitle),
            const SizedBox(height: 24),
            DropdownButtonFormField<String>(
              initialValue: _city,
              decoration: InputDecoration(labelText: l10n.demandSurveyCityLabel, border: const OutlineInputBorder()),
              hint: Text(l10n.demandSurveySelectCity),
              items: cities.map((city) => DropdownMenuItem(value: city, child: Text(city))).toList(),
              onChanged: (value) => setState(() => _city = value),
            ),
            const SizedBox(height: 20),
            ...data.options.map((option) => CheckboxListTile(
              contentPadding: EdgeInsets.zero,
              value: _selected.contains(option.id),
              title: Text(option.name),
              onChanged: (checked) => setState(() { checked == true ? _selected.add(option.id) : _selected.remove(option.id); }),
            )),
            const SizedBox(height: 12),
            TextField(controller: _otherController, maxLength: 500, maxLines: 4, decoration: InputDecoration(labelText: l10n.demandSurveyOtherOption, hintText: l10n.demandSurveyOtherHint, border: const OutlineInputBorder())),
            ...
            FilledButton(onPressed: _submitting ? null : _submit, child: _submitting ? const SizedBox.square(dimension: 20, child: CircularProgressIndicator(strokeWidth: 2)) : Text(l10n.demandSurveySubmit)),
          ],
        ),
      ),
    );
