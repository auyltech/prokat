...
| 3.5  | Открыть чат → background ~10 с (собеседник пишет) → resume с того же экрана, **не** через баннер.                                                        | Сообщения из фона видны без ручного refresh; push **этого** чата исчезает из шторки.             |
| 3.5a | Свернуть приложение вне чата (список/главная) → получить push от собеседника → открыть приложение и зайти в этот чат через навигацию, не нажимая баннер. | Push **этого** чата исчезает из шторки при входе на экран; чужие пуши остаются.                  |
...
| 3.8  | Иметь чат с непрочитанными; получить новое сообщение, **не** открывая этот чат.                                                                          | Preview обновился; **badge непрочитанных не обнуляется** только из-за обновления lastMessage.    |
...
| ChatModel newMessagesCount copyWith (RF-03)       | **3.3**, **3.6**, **3.8**                                       |
...
- Обновление lastMessage в списке чатов обнуляет badge непрочитанных, пока чат не открыт.
